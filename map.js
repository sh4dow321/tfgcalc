...
let worker=null,lastMap=null,drag=null;
...
function generate(){
  ...
  loading.className='map-loading busy';loading.innerHTML='<b>Generowanie mapy…</b><span>0%</span>';generateButton.disabled=true;$('#downloadMap').disabled=true;setStatus(location.protocol==='file:'?'Tryb lokalny — generowanie bez Workera…':'Obliczanie warstwy w osobnym wątku…','progress');updateHeader(settings);
  const handleData=data=>{
    if(data.type==='progress'){loading.querySelector('span').textContent=`${Math.round(data.value*100)}%`;return}
    if(data.type==='error'){loading.className='map-loading';loading.innerHTML='<b>Nie udało się wygenerować mapy</b><span>Sprawdź seed i spróbuj ponownie.</span>';generateButton.disabled=false;setStatus(data.message,'bad');return}
    if(data.type!=='result')return;
    canvas.width=data.width;canvas.height=data.height;ctx.putImageData(new ImageData(data.pixels,data.width,data.height),0,0);lastMap={...data,settings};loading.classList.add('hidden');generateButton.disabled=false;$('#downloadMap').disabled=false;setStatus(`Gotowe — ${data.width.toLocaleString('pl-PL')} × ${data.height.toLocaleString('pl-PL')} próbek. Wersja ${settings.version}.`,'ok');
    const query=new URLSearchParams({v:settings.version,seed:settings.seed,layer:settings.layer,x:settings.centerX,z:settings.centerZ,span:settings.span});try{history.replaceState(null,'',`${location.pathname}?${query}`)}catch{}
  };
  const generateLocally=()=>{if(fallbackStarted)return;fallbackStarted=true;worker?.terminate();worker=null;setStatus('Worker jest niedostępny — generowanie lokalne w głównym skrypcie…','progress');setTimeout(()=>{try{handleData(TFGMapCore.generateMap({...settings,width,height},value=>handleData({type:'progress',value})))}catch(error){handleData({type:'error',message:error?.message||String(error)})}},30)};
  if(location.protocol==='file:'||typeof Worker==='undefined'){generateLocally();return}
  try{
    worker=new Worker('map-worker.js?v=032');
    worker.onmessage=event=>handleData(event.data);
    worker.onerror=()=>generateLocally();
    worker.postMessage({...settings,width,height});
...
viewport.addEventListener('pointermove',event=>{
  if(drag){drag.lastX=event.clientX;drag.lastY=event.clientY;return}
