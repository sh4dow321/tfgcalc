...
function generate(){
  ...
  const finish=data=>{pendingMap.quartReady=data.quartReady;pendingMap.riverRasterReady=data.riverRasterReady;pendingMap.exactRocksReady=data.exactRocksReady;pendingMap.usedWasm=!!data.usedWasm;lastMap=pendingMap;pendingMap=null;workerBusy=false;loading.classList.add('hidden');generateButton.disabled=false;$('#downloadMap').disabled=false;setStatus(`Gotowe — ${width.toLocaleString('pl-PL')} × ${height.toLocaleString('pl-PL')} próbek · ${data.usedWasm?'WASM':'JS fallback'}. Wersja ${settings.version}.`,'ok');const query=new URLSearchParams({v:settings.version,seed:settings.seed,layer:settings.layer,x:settings.centerX,z:settings.centerZ,span:settings.span});try{history.replaceState(null,'',`${location.pathname}?${query}`)}catch{}};
