importScripts('./fastnoise-lite.js?v=035','./map-core.js?v=035','./region-v20.js?v=035');
importScripts('./map-wasm.js?v=035');
...
self.onmessage=async event=>{
  ...
  try{
    ...
    for(let y=0;y<fullHeight;y+=tileSize)for(let x=0;x<fullWidth;x+=tileSize){
      const width=Math.min(tileSize,fullWidth-x),height=Math.min(tileSize,fullHeight-y),sampleX=Math.max(0,x-1),sampleY=Math.max(0,y-1),sampleRight=Math.min(fullWidth-1,x+width),sampleBottom=Math.min(fullHeight-1,y+height),sampleWidth=sampleRight-sampleX+1,sampleHeight=sampleBottom-sampleY+1,x0=data.centerX+(sampleX/(fullWidth-1)-.5)*data.span,x1=data.centerX+(sampleRight/(fullWidth-1)-.5)*data.span,z0=data.centerZ+(sampleY/(fullHeight-1)-.5)*fullVerticalSpan,z1=data.centerZ+(sampleBottom/(fullHeight-1)-.5)*fullVerticalSpan,span=Math.max(1,x1-x0),verticalSpan=Math.max(1,z1-z0),tileData={...data,centerX:(x0+x1)/2,centerZ:(z0+z1)/2,span,verticalSpan,width:sampleWidth,height:sampleHeight,displaySpan:data.span,skipRender:!!wasm,skipExactRocks:!!wasm},generated=TFGMapCore.generateMap(tileData);
      if(wasm&&data.layer==='rocks')TFGWasm.fillRocks(generated,TFGMapCore.prepareRockTask(tileData));
      if(wasm)generated.pixels=TFGWasm.render(generated,data.layer)||TFGMapCore.renderPixels(data.layer,generated.width,generated.height,generated.temperatures,generated.rainfalls,generated.continents,generated.surfaces,generated.elevations,generated.rockCodes,generated.riverMasks,generated.kaolinMasks);
      const result=crop(generated,x-sampleX,y-sampleY,width,height);
    ...
    self.postMessage({type:'complete',requestId,width:fullWidth,height:fullHeight,quartReady:data.layer==='kaolin',riverRasterReady:data.layer==='rivers',exactRocksReady:data.layer==='rocks'&&data.version==='1.20.1',usedWasm:!!wasm});
